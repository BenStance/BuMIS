...
export function InventoryPage() {
  ...
  const openStockDrawer = (productId) => {
    setSelectedProductId(typeof productId === 'string' ? productId.trim() : productId)
    setStockData(null)
  ...
  const saveMovement = async (event) => {
    ...
    try {
      const productId = movementForm.productId.trim()
      if (!isGuid(productId)) {
      ...
      if (movementType === STOCK_IN) {
        await inventoryApi.stockIn({
          ...
          vendorId: isGuid(movementForm.vendorId) ? movementForm.vendorId.trim() : undefined,
        ...
        await inventoryApi.adjustment({
          ...
          quantity: Number(movementForm.quantity),
          reason: movementForm.reason,
          approvedByUserId: isGuid(movementForm.approvedByUserId) ? movementForm.approvedByUserId.trim() : undefined,
          referenceNumber: movementForm.referenceNumber || undefined,
          remarks: movementForm.remarks || undefined,
        })
      }
      ...
      <Modal isOpen={movementOpen} onClose={closeMovement} title={movementType === STOCK_IN ? 'Stock In' : movementType === STOCK_OUT ? 'Stock Out' : 'Stock Adjustment'}>
        <form onSubmit={saveMovement} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <SelectField
                ...
                options={productOptions.map((product) => ({ value: String(product.id || '').trim(), label: `${product.productName} (${product.sku})` }))}
                required
              />
          <Field
            label="Quantity"
          ...
          {movementType === STOCK_IN && (
            <>
              ...
              <SelectField
                ...
                options={vendorOptions.map((vendor) => ({ value: String(vendor.id || '').trim(), label: vendor.name }))}
              />
              <Field
